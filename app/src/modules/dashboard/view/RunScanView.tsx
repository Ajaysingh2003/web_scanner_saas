...
function WebsiteFavicon({
  ...
  if (!faviconSrc || hasError) {
    return <Globe className={cn("size-5 text-slate-500", className)} />;
  }

  return (
    <div className={cn("relative size-6 shrink-0 overflow-hidden rounded", className)}>
      <Image
        src={faviconSrc}
        alt="Target website favicon"
        fill
        sizes="24px"
        className="object-contain"
        onError={() => setHasError(true)}
        unoptimized // Recommended for external dynamic CDN avatars/favicons
      />
    </div>
  );
}
...
export default function RunScanView() {
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-6 px-4 py-6 sm:px-6">
      ...
      {/* Target with Google CDN Favicon & Next.js Image */}
      <div className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div className="flex min-w-0 items-center gap-3">
            <span className="flex size-10 shrink-0 items-center justify-center rounded-lg border border-slate-100 bg-slate-50 shadow-2xs">
              <WebsiteFavicon url={target} className="size-6" />
            </span>
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <span className="truncate text-sm font-semibold text-slate-900">
